...
static UP<CHyprlandEventBus>&  eventBus()  { static auto p = makeUnique<CHyprlandEventBus>();  return p; }
static UP<CHyprlandRenderer>&  renderer()  { static auto p = makeUnique<CHyprlandRenderer>();  return p; }
static UP<CHyprlandInput>&     input()     { static auto p = makeUnique<CHyprlandInput>();     return p; }
static UP<CHyprlandConfig>&    config()    { static auto p = makeUnique<CHyprlandConfig>();    return p; }
static UP<CWidgetRenderer>&    widgetRenderer() { static auto p = makeUnique<CWidgetRenderer>(); return p; }
static UP<CWidgetInputManager>&      inputManager()   { static auto p = makeUnique<CWidgetInputManager>();   return p; }
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
    ...
    safety->init([&](const std::string& reason, eSafeMode mode) {
        config()->notify(std::string{"Safe mode: "} + safeModeName(mode) + " — " + reason, 10000, CHyprColor{0}, ICON_WARNING);
    });
    ...
    if (!result.success) {
        HyprlandAPI::addNotificationV2(handle,
                                       {{"text", std::string{"[hyprlooks] failed to initialize: "} + result.error}, {"time", uint64_t{10000}}, {"color", CHyprColor{0}}, {"icon", ICON_ERROR}});
        safety->disable();
    }
