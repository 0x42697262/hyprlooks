...
namespace Hyprlooks {
    ...
    Hyprutils::Math::Vector2D CBar::measure() const {
        return {0.F, m_height};
    }
    ...
    void CBar::layout(const Hyprutils::Math::CBox& available) {
        ...
        if (m_left && m_left->childCount() > 0) {
            const auto size = m_left->measure();
            m_left->layout({inner.x, inner.y + (inner.h - size.y) / 2.0, size.x, size.y});
        }

        if (m_center && m_center->childCount() > 0) {
            const auto size = m_center->measure();
            m_center->layout({inner.x + (inner.w - size.x) / 2.0, inner.y + (inner.h - size.y) / 2.0, size.x, size.y});
        }

        if (m_right && m_right->childCount() > 0) {
            const auto size = m_right->measure();
            m_right->layout({inner.x + inner.w - size.x, inner.y + (inner.h - size.y) / 2.0, size.x, size.y});
        }
    }
    ...
    void CBar::ensureSections() {
        ...
        m_left->setSelf(WP<IWidget>{m_left});
        m_center->setSelf(WP<IWidget>{m_center});
        m_right->setSelf(WP<IWidget>{m_right});
